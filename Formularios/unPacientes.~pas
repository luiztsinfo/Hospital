...
uses
  ...
  Controls, Forms, Dialogs, unConsultas, FMTBcd,
...
uses unPrincipal, unManPessoas;

procedure TfrmPacientes.BtnAlterarClick(Sender: TObject);
begin
  frmManPessoas := TfrmManPessoas.Create(self);

  inherited;

  frmManPessoas.iCategoria := 0;
  frmManPessoas.ShowModal;
  FreeAndNil(frmManPessoas);

  mostraTodos;
end;

procedure TfrmPacientes.BtnIncluirClick(Sender: TObject);
begin
  frmManPessoas := TfrmManPessoas.Create(self);
  inherited;
  frmManPessoas.iCategoria := 0;
          ...
          with qryDados, SQL do
            begin
              ...
              if RdBtnCodigo.Checked then
                begin
                  Add('SELECT * FROM pessoa WHERE codigo = :cod AND (categoria IS NULL OR categoria = 0)');
              ...
              if RdBtnNome.Checked then
                begin
                  Add('SELECT * FROM pessoa WHERE nome LIKE :nome AND (categoria IS NULL OR categoria = 0)');
              ...
              if RdBtnCartaoSUS.Checked then
                begin
                  Add('SELECT * FROM pessoa WHERE cartaosus LIKE :sus AND (categoria IS NULL OR categoria = 0)');
              ...
              if RdBtnCPF.Checked then
                begin
                  Add('SELECT * FROM pessoa WHERE cpf LIKE :cpf AND (categoria IS NULL OR categoria = 0)');
              ...
              if RdBtnRG.Checked then
                begin
                  Add('SELECT * FROM pessoa WHERE rg LIKE :rg AND (categoria IS NULL OR categoria = 0)');
...
begin
  ...
  sSQL := 'SELECT * FROM pessoa WHERE categoria IS NULL OR categoria = 0 ORDER BY '+frmPrincipal.sCampo;
